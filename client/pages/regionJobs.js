  ...
  return (
    <section className={styles.workers}>
            ...
            <div className={styles.worker_main}>
                <div className={styles.row}>
                    {data.region.map((item, i) => {
                        const {_id, jobTitle, experience, region, age, create_at, salary} = item
                                ...
                                <div className={styles.card}>
                                    ...
                                    <div className={styles.card_body}>
                                        <ul>
                                            ...
                                            <li><strong>Yoshi:</strong>{age}</li>
                                            <div className={styles.end}>
                                                {create_at && <span className={styles.date}>{date}</span>}
                                                {!create_at && <span></span>}
                                                <Link href={{
                                                    pathname: `/ishlar/id/${i + 1}`,
                                                    query: {worker_id: _id}
                                                }}>
                                                    <a>
                                                        Batafsil
        ...
        {data.total===0 && <div style={{height:"30vh"}} className="container">
            <h1>Ish izlash <span>{region}</span></h1>
...
export async function getServerSideProps(context) {
    ...
    const res = await fetch(`${BASE_URL}jobs/region/${region}?page=${id}`)
