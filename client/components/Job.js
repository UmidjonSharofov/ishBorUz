...
const Job = () => {
    ...
    const fetchData = (token) => {
        setPageLoad(true)
        axios.get(`${BASE_URL}jobs/notAllow`,{
            headers:{
        ...
        <section className={styles.dashboard_job}>
            {data.length>=1 &&<table>
                ...
                {data.length >=1 && data?.map((item,i)=>{
                    const {_id,jobTitle,professionInfo,experience,salary,jobType,region,age,gender,phone,isShow}=item
                    return  <tr key={i}>
                                    <td>{i+1}</td>
                                    <td>{jobTitle}</td>
